...
conn = psycopg2.connect(host=DBHOST, dbname=DBNAME, user=DBUSER, password=DBPASS, connect_timeout=10, options="-c statement_timeout=10000 -c idle_in_transaction_session_timeout=60000 -c lock_timeout=5000")
conn.autocommit = True
c = conn.cursor()
c.execute("SET statement_timeout=10000;")  # 10s
c.execute("SET idle_in_transaction_session_timeout=60000;")  # 60s
c.execute("SET statement_timeout=10000;")  # 10s
c.execute("SET idle_in_transaction_session_timeout=60000;")  # 60s
c.execute("""
...
for row in r:
    ...
    c.execute("""
        ...
        VALUES (%s,%s,%s,%s,%s,%s,%s,'nasa_firms')
    """, (ts, lat, lon, sat, conf, frp, json.dumps(row)))

conn.commit()
c.close()
conn.close()
if rows:
    execute_values(c, "INSERT INTO air.fires(detected_at, lat, lon, satellite, confidence, frp, raw, source_code) VALUES %s",
